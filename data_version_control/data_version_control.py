...
def add_csv_files_to_dvc() -> None:
    ...
    sp.run("dvc add data/*.csv", shell=True, cwd=PROJECT_ROOT_DIR)


def add_dvc_files_to_git() -> None:
    """Add *.dvc files in the data/ directory to git."""
    sp.run("git add data/*.dvc", shell=True, cwd=PROJECT_ROOT_DIR)
...
def commit_new_dcv_changes() -> None:
    ...
    sp.run('git commit -m "Update the scraped data"', shell=True, cwd=PROJECT_ROOT_DIR)
