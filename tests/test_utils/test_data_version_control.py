...
@pytest.fixture
def mock_git_tag_exists():
    """Mock function _git_tag_exists.

    Yields:
        mock_git_tag_exists (mock.MagicMock): Mock function _git_tag_exists.
    """
    with mock.patch(
        "utils.data_version_control._git_tag_exists"
    ) as mock_git_tag_exists:
        yield mock_git_tag_exists


@pytest.fixture
def mock_git_commit_hash_exists():
    """Mock function _git_commit_hash_exists.

    Yields:
        mock_git_commit_hash_exists (mock.MagicMock): Mock function _git_commit_hash_exists.
    """
    with mock.patch(
        "utils.data_version_control._git_commit_hash_exists"
    ) as mock_git_commit_hash_exists:
        yield mock_git_commit_hash_exists
...
def test_git_checkout_folder_raises_file_not_found_error_without_directory(
    mock_git_command, mock_git_tag_exists, mock_git_commit_hash_exists
):
...
def test_git_commit_hash_exists(mock_git_repo):
...
def test_git_commit_hash_exists_returns_false_when_tag_does_not_exist(mock_git_repo):
